...
export default function AlertDetailModal({
  ...
  // Reset visible count whenever the modal opens
  useEffect(() => {
    if (open) {
  ...
  const loadMore = () => {
    ...
    // Small artificial delay to show the loader briefly — feels more responsive
    setTimeout(() => {
      ...
      <DialogContent className="bg-deep-indigo border-void-navy text-starlight max-w-2xl max-h-[85vh] flex flex-col">
        <DialogHeader>
          ...
          <DialogDescription className="text-faint-star">
            Official notices from NOAA. Scroll down to see all alerts — grouped
            by recency.
          </DialogDescription>
        </DialogHeader>
...
function AlertItem({ alert }: { alert: Alert }) {
  ...
  return (
    <div className="rounded-lg border border-void-navy bg-void-navy/50 p-4 transition-colors hover:bg-void-navy/70">
      <div className="flex items-start gap-2">
        <AlertTriangle size={16} className="mt-0.5 shrink-0 text-solar-amber" />
          ...
          <div className="mt-2 flex items-center justify-between gap-3">
            ...
            {needsTruncation && (
              <button
                ...
                className="text-xs text-aurora-green hover:underline shrink-0"
              >
